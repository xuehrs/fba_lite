...
static int InpDIPSWListBegin()
{
    ...
    LvCol.cx = 158;
    LvCol.pszText = LoadStringEx(hAppInst, IDS_DIP_GROUP, true);
    SendMessage(hInpDIPSWList, LVM_INSERTCOLUMN, 0, (LPARAM)&LvCol);
    LvCol.cx = 215;
    LvCol.pszText = LoadStringEx(hAppInst, IDS_DIP_SETTING, true);
...
int InpDIPSWCreate()
{
    ...
    DialogBox(hAppInst, MAKEINTRESOURCE(IDD_INPDIP), hMainWnd, (DLGPROC)DialogProc);
