...
static int __cdecl BzipBurnLoadRom(unsigned char *Dest, int *pnWrote, int i)
{
    ...
    if (pszRomName == NULL)
    {
        ...
        _stprintf(szTempName, _T("%s"), LoadStringEx(hAppInst, IDS_ERR_UNKNOWN, true));
        sprintf(pszRomName, "%s", TCHARToANSI(szTempName, NULL, 0));
    }

    TCHAR szTempLoading[100];
    _stprintf(szTempLoading, _T("%s"), LoadStringEx(hAppInst, IDS_PROGRESS_LOADING_ONLY, true));
    ...
    if (ri.nType & (BRF_PRG | BRF_GRA | BRF_SND | BRF_BIOS))
    {
        if (ri.nType & BRF_BIOS)
        {
            ...
            _stprintf(szTempBios, _T("%s"), LoadStringEx(hAppInst, IDS_ERR_LOAD_DET_BIOS, true));
            _stprintf(szText + _tcslen(szText), _T(" %s"), szTempBios);
        }
        if (ri.nType & BRF_PRG)
        {
            TCHAR szTempPrg[100];
            _stprintf(szTempPrg, _T("%s"), LoadStringEx(hAppInst, IDS_ERR_LOAD_DET_PRG, true));
            _stprintf(szText + _tcslen(szText), _T(" %s"), szTempPrg);
        }
        if (ri.nType & BRF_GRA)
        {
            TCHAR szTempGra[100];
            _stprintf(szTempGra, _T("%s"), LoadStringEx(hAppInst, IDS_ERR_LOAD_DET_GRA, true));
            _stprintf (szText + _tcslen(szText), _T(" %s"), szTempGra);
        }
        if (ri.nType & BRF_SND)
        {
            TCHAR szTempSnd[100];
            _stprintf(szTempSnd, _T("%s"), LoadStringEx(hAppInst, IDS_ERR_LOAD_DET_SND, true));
