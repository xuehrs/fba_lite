...
static void wr_inval(adsp2100_state *adsp, INT32 val)
{
    logerror( "ADSP %04x: Writing to an invalid register!\n", adsp->ppc );
}
static void wr_ax0(adsp2100_state *adsp, INT32 val)
{
    adsp->core.ax0.s = val;
}
static void wr_ax1(adsp2100_state *adsp, INT32 val)
{
    adsp->core.ax1.s = val;
}
static void wr_mx0(adsp2100_state *adsp, INT32 val)
{
    adsp->core.mx0.s = val;
}
static void wr_mx1(adsp2100_state *adsp, INT32 val)
{
    adsp->core.mx1.s = val;
}
static void wr_ay0(adsp2100_state *adsp, INT32 val)
{
    adsp->core.ay0.s = val;
}
static void wr_ay1(adsp2100_state *adsp, INT32 val)
{
    adsp->core.ay1.s = val;
}
static void wr_my0(adsp2100_state *adsp, INT32 val)
{
    adsp->core.my0.s = val;
}
static void wr_my1(adsp2100_state *adsp, INT32 val)
{
    adsp->core.my1.s = val;
}
static void wr_si(adsp2100_state *adsp, INT32 val)
{
    adsp->core.si.s = val;
}
static void wr_se(adsp2100_state *adsp, INT32 val)
{
    adsp->core.se.s = (INT8)val;
}
static void wr_ar(adsp2100_state *adsp, INT32 val)
{
    adsp->core.ar.s = val;
}
static void wr_mr0(adsp2100_state *adsp, INT32 val)
{
    adsp->core.mr.mrx.mr0.s = val;
}
static void wr_mr1(adsp2100_state *adsp, INT32 val)
{
    adsp->core.mr.mrx.mr1.s = val;
    adsp->core.mr.mrx.mr2.s = (INT16)val >> 15;
}
static void wr_mr2(adsp2100_state *adsp, INT32 val)
{
    adsp->core.mr.mrx.mr2.s = (INT8)val;
}
static void wr_sr0(adsp2100_state *adsp, INT32 val)
{
    adsp->core.sr.srx.sr0.s = val;
}
static void wr_sr1(adsp2100_state *adsp, INT32 val)
{
    adsp->core.sr.srx.sr1.s = val;
}
static void wr_i0(adsp2100_state *adsp, INT32 val)
{
    adsp->i[0] = val & 0x3fff;
    update_i(adsp, 0);
}
static void wr_i1(adsp2100_state *adsp, INT32 val)
{
    adsp->i[1] = val & 0x3fff;
    update_i(adsp, 1);
}
static void wr_i2(adsp2100_state *adsp, INT32 val)
{
    adsp->i[2] = val & 0x3fff;
    update_i(adsp, 2);
}
static void wr_i3(adsp2100_state *adsp, INT32 val)
{
    adsp->i[3] = val & 0x3fff;
    update_i(adsp, 3);
}
static void wr_i4(adsp2100_state *adsp, INT32 val)
{
    adsp->i[4] = val & 0x3fff;
    update_i(adsp, 4);
}
static void wr_i5(adsp2100_state *adsp, INT32 val)
{
    adsp->i[5] = val & 0x3fff;
    update_i(adsp, 5);
}
static void wr_i6(adsp2100_state *adsp, INT32 val)
{
    adsp->i[6] = val & 0x3fff;
    update_i(adsp, 6);
}
static void wr_i7(adsp2100_state *adsp, INT32 val)
{
    adsp->i[7] = val & 0x3fff;
    update_i(adsp, 7);
}
static void wr_m0(adsp2100_state *adsp, INT32 val)
{
    adsp->m[0] = (INT32)(val << 18) >> 18;
}
static void wr_m1(adsp2100_state *adsp, INT32 val)
{
    adsp->m[1] = (INT32)(val << 18) >> 18;
}
static void wr_m2(adsp2100_state *adsp, INT32 val)
{
    adsp->m[2] = (INT32)(val << 18) >> 18;
}
static void wr_m3(adsp2100_state *adsp, INT32 val)
{
    adsp->m[3] = (INT32)(val << 18) >> 18;
}
static void wr_m4(adsp2100_state *adsp, INT32 val)
{
    adsp->m[4] = (INT32)(val << 18) >> 18;
}
static void wr_m5(adsp2100_state *adsp, INT32 val)
{
    adsp->m[5] = (INT32)(val << 18) >> 18;
}
static void wr_m6(adsp2100_state *adsp, INT32 val)
{
    adsp->m[6] = (INT32)(val << 18) >> 18;
}
static void wr_m7(adsp2100_state *adsp, INT32 val)
{
    adsp->m[7] = (INT32)(val << 18) >> 18;
}
static void wr_l0(adsp2100_state *adsp, INT32 val)
{
    adsp->l[0] = val & 0x3fff;
    update_l(adsp, 0);
}
static void wr_l1(adsp2100_state *adsp, INT32 val)
{
    adsp->l[1] = val & 0x3fff;
    update_l(adsp, 1);
}
static void wr_l2(adsp2100_state *adsp, INT32 val)
{
    adsp->l[2] = val & 0x3fff;
    update_l(adsp, 2);
}
static void wr_l3(adsp2100_state *adsp, INT32 val)
{
    adsp->l[3] = val & 0x3fff;
    update_l(adsp, 3);
}
static void wr_l4(adsp2100_state *adsp, INT32 val)
{
    adsp->l[4] = val & 0x3fff;
    update_l(adsp, 4);
}
static void wr_l5(adsp2100_state *adsp, INT32 val)
{
    adsp->l[5] = val & 0x3fff;
    update_l(adsp, 5);
}
static void wr_l6(adsp2100_state *adsp, INT32 val)
{
    adsp->l[6] = val & 0x3fff;
    update_l(adsp, 6);
}
static void wr_l7(adsp2100_state *adsp, INT32 val)
{
    adsp->l[7] = val & 0x3fff;
    update_l(adsp, 7);
}
static void wr_astat(adsp2100_state *adsp, INT32 val)
{
    adsp->astat = val & 0x00ff;
}
static void wr_mstat(adsp2100_state *adsp, INT32 val)
{
    adsp->mstat = val & adsp->mstat_mask;
    update_mstat(adsp);
}
static void wr_imask(adsp2100_state *adsp, INT32 val)
{
    adsp->imask = val & adsp->imask_mask;
    check_irqs(adsp);
}
static void wr_icntl(adsp2100_state *adsp, INT32 val)
{
    adsp->icntl = val & 0x001f;
    check_irqs(adsp);
}
static void wr_cntr(adsp2100_state *adsp, INT32 val)
{
    cntr_stack_push(adsp);
    adsp->cntr = val & 0x3fff;
}
static void wr_sb(adsp2100_state *adsp, INT32 val)
{
    adsp->core.sb.s = (INT32)(val << 27) >> 27;
}
static void wr_px(adsp2100_state *adsp, INT32 val)
{
    adsp->px = val;
}
...
static void wr_tx0(adsp2100_state *adsp, INT32 val)
{
    if (adsp->sport_tx_callback) (*adsp->sport_tx_callback)(adsp, 0, val);
}
static void wr_tx1(adsp2100_state *adsp, INT32 val)
{
    if (adsp->sport_tx_callback) (*adsp->sport_tx_callback)(adsp, 1, val);
}
static void wr_owrctr(adsp2100_state *adsp, INT32 val)
{
    adsp->cntr = val & 0x3fff;
}
static void wr_topstack(adsp2100_state *adsp, INT32 val)
{
    pc_stack_push_val(adsp, val & 0x3fff);
}
...
static INT32 rd_inval(adsp2100_state *adsp)
{
    logerror( "ADSP %04x: Writing to an invalid register!\n", adsp->ppc );
    return 0;
}
static INT32 rd_ax0(adsp2100_state *adsp)
{
    return adsp->core.ax0.s;
}
static INT32 rd_ax1(adsp2100_state *adsp)
{
    return adsp->core.ax1.s;
}
static INT32 rd_mx0(adsp2100_state *adsp)
{
    return adsp->core.mx0.s;
}
static INT32 rd_mx1(adsp2100_state *adsp)
{
    return adsp->core.mx1.s;
}
static INT32 rd_ay0(adsp2100_state *adsp)
{
    return adsp->core.ay0.s;
}
static INT32 rd_ay1(adsp2100_state *adsp)
{
    return adsp->core.ay1.s;
}
static INT32 rd_my0(adsp2100_state *adsp)
{
    return adsp->core.my0.s;
}
static INT32 rd_my1(adsp2100_state *adsp)
{
    return adsp->core.my1.s;
}
static INT32 rd_si(adsp2100_state *adsp)
{
    return adsp->core.si.s;
}
static INT32 rd_se(adsp2100_state *adsp)
{
    return adsp->core.se.s;
}
static INT32 rd_ar(adsp2100_state *adsp)
{
    return adsp->core.ar.s;
}
static INT32 rd_mr0(adsp2100_state *adsp)
{
    return adsp->core.mr.mrx.mr0.s;
}
static INT32 rd_mr1(adsp2100_state *adsp)
{
    return adsp->core.mr.mrx.mr1.s;
}
static INT32 rd_mr2(adsp2100_state *adsp)
{
    return adsp->core.mr.mrx.mr2.s;
}
static INT32 rd_sr0(adsp2100_state *adsp)
{
    return adsp->core.sr.srx.sr0.s;
}
static INT32 rd_sr1(adsp2100_state *adsp)
{
    return adsp->core.sr.srx.sr1.s;
}
static INT32 rd_i0(adsp2100_state *adsp)
{
    return adsp->i[0];
}
static INT32 rd_i1(adsp2100_state *adsp)
{
    return adsp->i[1];
}
static INT32 rd_i2(adsp2100_state *adsp)
{
    return adsp->i[2];
}
static INT32 rd_i3(adsp2100_state *adsp)
{
    return adsp->i[3];
}
static INT32 rd_i4(adsp2100_state *adsp)
{
    return adsp->i[4];
}
static INT32 rd_i5(adsp2100_state *adsp)
{
    return adsp->i[5];
}
static INT32 rd_i6(adsp2100_state *adsp)
{
    return adsp->i[6];
}
static INT32 rd_i7(adsp2100_state *adsp)
{
    return adsp->i[7];
}
static INT32 rd_m0(adsp2100_state *adsp)
{
    return adsp->m[0];
}
static INT32 rd_m1(adsp2100_state *adsp)
{
    return adsp->m[1];
}
static INT32 rd_m2(adsp2100_state *adsp)
{
    return adsp->m[2];
}
static INT32 rd_m3(adsp2100_state *adsp)
{
    return adsp->m[3];
}
static INT32 rd_m4(adsp2100_state *adsp)
{
    return adsp->m[4];
}
static INT32 rd_m5(adsp2100_state *adsp)
{
    return adsp->m[5];
}
static INT32 rd_m6(adsp2100_state *adsp)
{
    return adsp->m[6];
}
static INT32 rd_m7(adsp2100_state *adsp)
{
    return adsp->m[7];
}
static INT32 rd_l0(adsp2100_state *adsp)
{
    return adsp->l[0];
}
static INT32 rd_l1(adsp2100_state *adsp)
{
    return adsp->l[1];
}
static INT32 rd_l2(adsp2100_state *adsp)
{
    return adsp->l[2];
}
static INT32 rd_l3(adsp2100_state *adsp)
{
    return adsp->l[3];
}
static INT32 rd_l4(adsp2100_state *adsp)
{
    return adsp->l[4];
}
static INT32 rd_l5(adsp2100_state *adsp)
{
    return adsp->l[5];
}
static INT32 rd_l6(adsp2100_state *adsp)
{
    return adsp->l[6];
}
static INT32 rd_l7(adsp2100_state *adsp)
{
    return adsp->l[7];
}
static INT32 rd_astat(adsp2100_state *adsp)
{
    return adsp->astat;
}
static INT32 rd_mstat(adsp2100_state *adsp)
{
    return adsp->mstat;
}
static INT32 rd_sstat(adsp2100_state *adsp)
{
    return adsp->sstat;
}
static INT32 rd_imask(adsp2100_state *adsp)
{
    return adsp->imask;
}
static INT32 rd_icntl(adsp2100_state *adsp)
{
    return adsp->icntl;
}
static INT32 rd_cntr(adsp2100_state *adsp)
{
    return adsp->cntr;
}
static INT32 rd_sb(adsp2100_state *adsp)
{
    return adsp->core.sb.s;
}
static INT32 rd_px(adsp2100_state *adsp)
{
    return adsp->px;
}
static INT32 rd_rx0(adsp2100_state *adsp)
{
    if (adsp->sport_rx_callback) return (*adsp->sport_rx_callback)(adsp, 0);
    else return 0;
}
static INT32 rd_rx1(adsp2100_state *adsp)
{
    if (adsp->sport_rx_callback) return (*adsp->sport_rx_callback)(adsp, 1);
    else return 0;
}
static INT32 rd_stacktop(adsp2100_state *adsp)
{
    return pc_stack_pop_val(adsp);
}
