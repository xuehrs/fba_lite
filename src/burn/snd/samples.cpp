...
static void make_raw(UINT8 *src, UINT32 len)
{
    ...
    UINT32 length = get_long();
    ptr += 4; // total length of file
    if (len < length) length = len - 8;		  // first 8 bytes (RIFF + Len)

    /* "WAVEfmt " */			ptr += 8; // WAVEfmt + 1 space
    UINT32 length2 = get_long();
    ptr += 4; // Wavefmt length
    /*	unsigned short format = get_short();  */ptr += 2; // format?
    UINT16 channels = get_short();
    ptr += 2; // channels
    UINT32 sample_rate = get_long();
    ptr += 4; // sample rate
    ...
    UINT16 bits = get_short() / 8;
    ptr += 2; // bits per sample	(0010)
    ...
    if (ptr[0] != 'd' || ptr[1] != 'a' || ptr[2] != 't' || ptr[3] != 'a')
    {
        ...
        UINT32 length3 = get_long();
        ptr += 4;
        ptr += length3;
    }

    /* "data" */				ptr += 4; // "data"
    UINT32 data_length = get_long();
    ptr += 4; // should be up to the data...
